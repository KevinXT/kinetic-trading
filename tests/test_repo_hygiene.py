...
def test_check_build_pollution_detects_package_build(tmp_path: Path) -> None:
    _run("clean_build.sh", check=True)
    build_dir = REPO_ROOT / "packages" / "common" / "build" / "lib"
    ...
    try:
        ...
        assert "packages/common/build" in result.stderr
    finally:
        _run("clean_build.sh", check=True)
