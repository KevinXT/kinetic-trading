...
from kinetic.core.config import apply_local_overrides, load_config
from kinetic.core.errors import DataSourceError
from kinetic.ingestion.warehouse.bigquery.client import SafeBigQueryClient

CASE_STUDY_CONFIGS = "projects/semiconductor_case_study/configs"

DRYRUN = f"{CASE_STUDY_CONFIGS}/semiconductors_seeded_theme_scoring_30d_dryrun.yaml"
EXECUTE = f"{CASE_STUDY_CONFIGS}/semiconductors_seeded_theme_scoring_30d_execute.yaml"


def _params(path: str) -> dict:
    """Params of the config's single step, in the authoritative `steps` shape."""
    steps = load_config(path)["pipeline"]["steps"]
    assert len(steps) == 1, f"{path} is expected to be a single-step config"
    assert steps[0]["task"] == "news.gdelt.bigquery.score_seeded_themes"
    return dict(steps[0]["params"])
