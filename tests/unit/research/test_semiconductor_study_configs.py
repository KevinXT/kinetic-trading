...
CASE_STUDY_CONFIGS = Path("projects/semiconductor_case_study/configs")
...
def _params(cfg: dict) -> dict:
    """Params of the config's single step, in the authoritative `steps` shape."""
    steps = (cfg.get("pipeline", {}) or {}).get("steps", []) or []
    return dict(steps[0].get("params", {})) if steps else {}


def _task(cfg: dict) -> str:
    steps = (cfg.get("pipeline", {}) or {}).get("steps", []) or []
    return str(steps[0].get("task", "")) if steps else ""


# Aliases: this file originally read the legacy ingest/strategy split. Every
# config here is single-step, so both roles resolve to the same step's params.
_ingest = _params
_strategy = _params
...
def test_all_study_configs_exist() -> None:
    for name in (
        ...
        assert (CASE_STUDY_CONFIGS / name).is_file(), f"missing config {name}"


@pytest.mark.parametrize("name", [THEME_DRYRUN, THEME_EXECUTE])
def test_theme_discovery_configs(name: str) -> None:
    cfg = load_yaml(CASE_STUDY_CONFIGS / name)
    assert _task(cfg) == "news.gdelt.bigquery.discover_themes"
    ingest = _ingest(cfg)
...
def test_theme_discovery_execute_requires_typed_gate() -> None:
    dry = _ingest(load_yaml(CASE_STUDY_CONFIGS / THEME_DRYRUN)).get("cost_controls", {})
    ex = _ingest(load_yaml(CASE_STUDY_CONFIGS / THEME_EXECUTE)).get("cost_controls", {})
...
def test_count_configs_use_partition_and_daily_window(name: str) -> None:
    cfg = load_yaml(CASE_STUDY_CONFIGS / name)
    assert _task(cfg) == "news.gdelt.bigquery.fetch_daily_counts"
    ingest = _ingest(cfg)
    assert ingest.get("topic") == "semiconductors"
...
def test_count_execute_requires_typed_gate_and_dryrun_is_dry() -> None:
    dry = _ingest(load_yaml(CASE_STUDY_CONFIGS / COUNT_DRYRUN)).get("cost_controls", {})
    ex = _ingest(load_yaml(CASE_STUDY_CONFIGS / COUNT_EXECUTE)).get("cost_controls", {})
    assert dry.get("dry_run") is True and dry.get("execute_query", "") != "ENABLE"
    assert ex.get("dry_run") is False and ex.get("execute_query") == "ENABLE"
    for cc in (dry, ex):
        assert isinstance(cc.get("maximum_bytes_billed"), int) and cc["maximum_bytes_billed"] > 0
...
def test_alpaca_config_symbols_identity_and_no_credentials() -> None:
    cfg = load_yaml(CASE_STUDY_CONFIGS / ALPACA)
    ...
    raw = (CASE_STUDY_CONFIGS / ALPACA).read_text(encoding="utf-8")
...
def test_study_build_configs_are_fixed_and_conservative(name: str) -> None:
    cfg = load_yaml(CASE_STUDY_CONFIGS / name)
    assert _task(cfg) == "research.build_news_market_dataset"
    strat = _strategy(cfg)
...
def test_v1_consumes_executed_artifacts_not_raw_alignment() -> None:
    strat = _strategy(load_yaml(CASE_STUDY_CONFIGS / STUDY_V1))
