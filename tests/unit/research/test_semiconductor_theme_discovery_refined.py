...
def _params(cfg: dict) -> dict:
    """Params of the config's first step, in the authoritative `steps` shape."""
...
def test_refined_configs_exist() -> None:
    assert (CASE_STUDY_CONFIGS / REFINED_DRYRUN).is_file()
    assert (CASE_STUDY_CONFIGS / REFINED_EXECUTE).is_file()
...
def test_refined_configs_use_targeted_token_patterns(name: str) -> None:
    ingest = _params(load_yaml(CASE_STUDY_CONFIGS / name))
...
def test_refined_dryrun_and_execute_patterns_match() -> None:
    dry = _params(load_yaml(CASE_STUDY_CONFIGS / REFINED_DRYRUN)).get("theme_search_patterns")
    ex = _params(load_yaml(CASE_STUDY_CONFIGS / REFINED_EXECUTE)).get("theme_search_patterns")
    assert dry == ex
...
def test_refined_cost_controls() -> None:
    dry = _params(load_yaml(CASE_STUDY_CONFIGS / REFINED_DRYRUN)).get("cost_controls", {})
    ex = _params(load_yaml(CASE_STUDY_CONFIGS / REFINED_EXECUTE)).get("cost_controls", {})
...
def test_seeded_configs_exist() -> None:
    assert (CASE_STUDY_CONFIGS / SEEDED_DRYRUN).is_file()
    assert (CASE_STUDY_CONFIGS / SEEDED_EXECUTE).is_file()
...
def test_seeded_configs_shape_and_partition(name: str) -> None:
    ...
    assert _task(cfg) == "news.gdelt.bigquery.discover_seeded_themes"
    ingest = _params(cfg)
    assert ingest.get("topic") == "semiconductors"
...
def test_seeded_cost_controls() -> None:
    dry = _params(load_yaml(CASE_STUDY_CONFIGS / SEEDED_DRYRUN)).get("cost_controls", {})
    ex = _params(load_yaml(CASE_STUDY_CONFIGS / SEEDED_EXECUTE)).get("cost_controls", {})
    assert dry.get("dry_run") is True and dry.get("execute_query", "") != "ENABLE"
    assert ex.get("dry_run") is False and ex.get("execute_query") == "ENABLE"
    for cc in (dry, ex):
        assert isinstance(cc.get("maximum_bytes_billed"), int) and cc["maximum_bytes_billed"] > 0
