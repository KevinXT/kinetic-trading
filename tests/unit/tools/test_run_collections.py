...
from tools.run_collections import discover_configs, run_all, run_config
...
@patch("tools.run_collections.run_config")
@patch("tools.run_collections.time.sleep")
...
@patch("tools.run_collections.subprocess.run")
def test_run_config_invokes_the_kinetic_cli(mock_run: MagicMock) -> None:
    mock_run.return_value = subprocess.CompletedProcess(
    ...
    assert "-m" in args
    assert "kinetic.interface.cli.app" in args
    assert "run" in args
...
@patch("tools.run_collections.run_config")
@patch("tools.run_collections.time.sleep")
def test_all_failures_reported(mock_sleep: MagicMock, mock_run: MagicMock, tmp_path: Path) -> None:
