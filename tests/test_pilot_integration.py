...
def _load_params() -> dict:
    path = Path("configs/research/semiconductor_relevance_real_corpus_pilot_local.yaml")
    cfg = yaml.safe_load(path.read_text(encoding="utf-8"))
    return cfg["pipeline"]["strategy"]


def test_task_registered() -> None:
    import trading_platform  # noqa: F401

    assert "run_semiconductor_relevance_real_corpus_pilot" in TASK_REGISTRY
