...
from market_data.providers.alpaca.provider import AlpacaPriceProvider
...
def test_app_registers_alpaca_historical_bars_task() -> None:
...
def test_committed_example_uses_explicit_historical_iex_request() -> None:
    path = Path(__file__).parents[1] / "configs" / "alpaca_daily_bars.yaml"
    config = yaml.safe_load(path.read_text(encoding="utf-8"))
    ingest = config["pipeline"]["ingest"]
    assert ingest["source"] == "alpaca_historical_bars"
    assert ingest["request"]["feed"] == "iex"
    assert ingest["request"]["adjustment"] == "all"
