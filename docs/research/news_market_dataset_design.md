...
Scope owner: research-data layer (`packages/research_data`).
...
- **DOC / article-list path** (`news_data.gdelt`): article-level records with
...
- **BigQuery / GKG counts path** (`news_data.bigquery`): daily rows
...
Market path (`market_data`): `PriceBar(symbol, timestamp, timeframe, open, high,
