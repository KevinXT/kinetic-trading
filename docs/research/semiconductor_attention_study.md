...
`experiments/semiconductors_seeded_theme_candidate_analysis/` (run
`scripts/analyze_seeded_theme_candidates.py`) and the report it emits.

`seeded-v2` (`news_data.bigquery.gdelt_queries`, `seed_matching`) fixes matching
...
odds ratio are computable (`news_data.bigquery.theme_scoring`). It emits
...
3. Turn the discovery output into a review worksheet:
   ...
   python3 scripts/build_theme_review_worksheet.py \
     --input experiments/.../artifacts/theme_discovery.csv \
...
| Alpaca bars | `configs/research/semiconductors_alpaca_bars.yaml` | `semiconductors_alpaca_bars` | `data/processed/market/market_bars.jsonl` |
...
#    Read experiments/semiconductors_news_market_study_v1/semiconductors_study_v1/
