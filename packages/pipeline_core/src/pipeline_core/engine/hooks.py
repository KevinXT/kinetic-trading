...
subclasses. ``runner.run_plan`` orchestrates the loop and invokes hooks.
...
from pipeline_core.engine.parser import Step
from pipeline_core.engine.run_metadata import (
    git_head_sha_near,
    utc_now_iso,
    write_run_metadata,
)
...
class ErrorHook(Hook):
    ...
    def on_error(self, ctx: RunContext, step: Step, error: BaseException) -> None:
        ...
        if isinstance(error, Exception) and sys.exc_info()[0] is not None:
            (ctx.run_dir / "traceback.txt").write_text(
