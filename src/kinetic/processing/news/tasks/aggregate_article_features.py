...
def aggregate_article_features_task(ctx, params: Dict[str, Any]) -> None:
    ...
    for bucket_key in sorted(bucket_order):
        date, topic = bucket_key
        ...
        if dup_groups is not None:
            ...
            row["duplicate_articles_removed"] = sum(dg.get("duplicate_count", 0) for dg in matched)
