...
def _build_duplicate_groups(
    ...
    for key in group_order:
        ...
        dup_urls = [str(duplicate.get("url", "")) for duplicate in duplicates]

        dup_titles_seen: set[str] = set()
        dup_titles: List[str] = []
        for duplicate in duplicates:
            t = str(duplicate.get("title", ""))
...
def _resolve_strategy(params: Dict[str, Any]) -> str:
    ...
    if "dedupe_strategy" in params:
        strategy = str(params["dedupe_strategy"])
    ...
    if "by" in params:
        ...
        if set(by) == {"title", "domain"}:
            return "title_domain"
        logger.warning(
            "Legacy 'by' value %s does not map to a known strategy; " "falling back to '%s'.",
            by,
            _DEFAULT_STRATEGY,
        )
