...
_DEFAULT_OUTPUT_PATH = "warehouse/normalized/news/articles.jsonl"
...
def store_articles_task(ctx, params: Dict[str, Any]) -> None:
    ...
    for state_key in _INPUT_STATE_KEYS:
        if state_key in ctx.state:
            articles = ctx.state[state_key]
            input_state_used = state_key
            break
