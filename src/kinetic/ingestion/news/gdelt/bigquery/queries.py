...
from kinetic.data.catalog.seeds import SeedTerm, resolve_seed_terms
from kinetic.ingestion.news.gdelt.bigquery.seed_predicates import (
    matched_seeds_array_expr,
    seed_match_predicate,
)
from kinetic.ingestion.windows import (
...
#            industry phrases (see kinetic.data.catalog.seeds); per-record theme dedup +
...
def build_gdelt_seeded_theme_discovery_query(
    ...
    *records* carry them. The output is a human-review candidate list — frequency
    ...
    1. **Safe matching.** Company aliases are matched only as whole tokens (so
       ...
       substring matching. See :mod:`kinetic.data.catalog.seeds`.
