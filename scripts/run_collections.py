"""
Run all YAML collection configs through the trading platform pipeline.

Discovers configs under a root directory, executes each one sequentially
via ``python3 -m trading_platform <config>``, and prints a progress summary.
"""
...
def run_config(config_path: Path) -> subprocess.CompletedProcess[str]:
    """Run a single config through the trading-platform CLI."""
    return subprocess.run(
        [sys.executable, "-m", "trading_platform", str(config_path)],
        capture_output=True,
        text=True,
    )
